...
export default function HeaderClient({ navigation }) {
  ...
  return (
    <Bounded as="header" className="text-white bg-darkGray fixed w-full z-50">
      <motion.div
        className="flex justify-between items-center lg:p-0"
        initial="hidden"
        animate="visible"
      ...
      >
        ...
        <nav
          className={`fixed lg:static top-0 left-0 w-full h-full bg-darkGray transition-transform transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:transition-none`}
        >
          ...
          <ul className="flex flex-col lg:flex-row lg:gap-11 lg:items-center lg:justify-end h-full lg:h-auto">
            {navigation.map(({ link, label }) => (
              <li
                key={label}
                className="opacity-80 hover:opacity-100 transition-opacity duration-200 ease-in-out lg:my-0 my-3 text-center lg:text-left"
              >
                <PrismicNextLink
                  field={link}
                  className="py-4 tracking-wider text-white block lg:inline"
                  onClick={closeMenu}
                >
                  {label}
                </PrismicNextLink>
